...
error_flag DMAMEM __oe_mother_board_error_any;
...
static PROGMEM void canzero_handle_get_req(canzero_frame* frame) {
  ...
  case 10: {
    ...
    resp.m_header.m_eof = 0;
    resp.m_header.m_toggle = 0;
    schedule_get_resp_fragmentation_job(__oe_error_level_config_mcu_temperature_rx_fragmentation_buffer, 7, 10, msg.m_header.m_client_id);
    break;
  }
  case 11: {
  ...
  case 13: {
    resp.m_data |= ((uint32_t)(((uint8_t)__oe_mother_board_error_any) & (0xFF >> (8 - 1)))) << 0;
    resp.m_header.m_sof = 1;
    resp.m_header.m_eof = 1;
    resp.m_header.m_toggle = 0;
    break;
...
static PROGMEM void canzero_handle_set_req(canzero_frame* frame) {
  ...
  case 13 : {
    ...
    error_flag mother_board_error_any_tmp;
    mother_board_error_any_tmp = ((error_flag)((msg.m_data >> 0) & (0xFFFFFFFF >> (32 - 1))));
...
void canzero_init() {
  __oe_config_hash = 11735695416577481027ull;
  __oe_build_time = {
    .m_year = BUILD_YEAR,
    .m_month = BUILD_MONTH,
    .m_day = BUILD_DAY,
...
void canzero_send_error_level_config_mcu_temperature() {
  ...
  canzero_serialize_canzero_message_get_resp(&msg, &sender_frame);
  canzero_can0_send(&sender_frame);
  schedule_get_resp_fragmentation_job(__oe_error_level_config_mcu_temperature_send_fragmentation_buffer, 7, 10, 255);
...
void canzero_send_mother_board_error_any() {
  canzero_message_get_resp msg;
  msg.m_data |= ((uint32_t)(((uint8_t)__oe_mother_board_error_any) & (0xFF >> (8 - 1)))) << 0;
  msg.m_header.m_eof = 1;
  msg.m_header.m_sof = 1;
  msg.m_header.m_toggle = 0;
